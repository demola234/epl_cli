	...
	case tea.KeyMsg:
		...
		case "enter":
			return m, tea.Batch(
				...
				tea.Printf("Let's show the details for %s Points %s\n", m.table.SelectedRow()[1], m.table.SelectedRow()[2]),
			)
		}
	case errorMsg:
		// Handle error
...
func fetchFixtures(teamId string) tea.Msg {
	done := make(chan bool)
	defer close(done)
	...
	// Start a goroutine to fetch data
	go func() {
		fixtureData, err := api.GetFixtureData(teamId)
		if err != nil {
			fmt.Println("Error getting data:")
			// Signal completion before returning
			done <- true
	...
	case fixtureData := <-ch:
		...
		for _, row := range fixtureData.Data.Fixtures {
			rows = append(rows, table.Row{row.HomeName, row.AwayName, row.Location, row.GoalsScored, row.GoalDiff, row.Matches})
		}
